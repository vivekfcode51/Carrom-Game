...
const BoardCanvas = ({
  ...
  const pockets = [
    { x: 70, y: 70 }, { x: 530, y: 70 },
    { x: 70, y: 530 }, { x: 530, y: 530 },
  ];
  ...
  const updateGame = () => {
    ...
    coinsList.forEach((coin) => {
      ...
      // Sunken effect
      const isPocketedVisually = pockets.some(p => Math.hypot(p.x - coin.x, p.y - coin.y) < 20);
...
const remaining = coinsList.filter((coin) => {
  ...
  return true;
    ...
    if (remaining.length !== coinsList.length) {
      ...
      setTurnRef.current(prev => (prev === "Player 1" ? "Player 2" : "Player 1"));
  ...
  const drawArrow = (ctx, x, y, dx, dy, length = 50) => {
    ...
    ctx.lineTo(endX - headLength * Math.cos(leftAngle), endY - headLength * Math.sin(leftAngle));
    ctx.lineTo(endX - headLength * Math.cos(rightAngle), endY - headLength * Math.sin(rightAngle));
    ctx.closePath();
