...
const BoardCanvas = ({
  ...
  const updateGame = () => {
    ...
    //   setCoins([...remaining]);
    // } else {
    //   // 🔁 No coin pocketed, switch turn
    //   setTurnRef.current((prev) =>
...
//       } else {
//         if (newQueenPocketed) coinCovered = true;
//         const currentTurn = turnRef.current;
//         setScores((prev) => {
//           const updated = { ...prev };
...
//       }
//     } else {
//       // Draw if not pocketed
//       const isNearPocket = pockets.some(p => Math.hypot(p.x - coin.x, p.y - coin.y) < 20);
//       const yOffset = isNearPocket ? 3 : 0;
//       const radius = coin.radius * (isNearPocket ? 0.85 : 1);
//       drawCircle(ctx, coin.x, coin.y + yOffset, radius, coin.color);
...
//   if (newQueenPocketed && !coinCovered) {
//     const queen = {
//       x: 300, y: 300,
//       radius: 10,
//       color: "red",
...
//       pocketed: false
//     };
//     updatedCoins.push(queen);
//     playSound("queen-return");
...
//   // ✅ STRIKER update (This was missing in your code)
//   // Move striker
// const s = strikerRef.current;
// s.vx *= 0.98;
// s.vy *= 0.98;
...
//   // Switch turn only when striker & all coins stop
//   const anyMoving = updatedCoins.some(c => Math.abs(c.vx) > 0.05 || Math.abs(c.vy) > 0.05);
//   const strikerMoving = Math.abs(s.vx) > 0.05 || Math.abs(s.vy) > 0.05;

//   if (!anyMoving && !strikerMoving) {
//     if (!coinPocketed) {
...
//       const newSpeed1 = (speed1 * (1 - 1) + 2 * speed2) / 2;
//       const newSpeed2 = (speed2 * (1 - 1) + 2 * speed1) / 2;
...
//     s.y = Math.max(min, Math.min(max, s.y));
//   };
